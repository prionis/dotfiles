...
vim.pack.add({ gh("ray-x/go.nvim"), gh("ray-x/guihua.lua"), gh("neovim/nvim-lspconfig") })
require("go").setup({
	ai = { enable = true },
})
...
do
	...
	local parsers =
		{ "go", "bash", "c", "diff", "html", "lua", "luadoc", "markdown", "markdown_inline", "query", "vim", "vimdoc" }
	...
	end, { desc = "jump to lower context", silent = true })
	vim.keymap.set("n", "[C", function()
		require("treesitter-context").go_to_context(999)
	end, { desc = "jump to highest context", silent = true })
end
